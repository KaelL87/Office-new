...
const categoryReducer = (state, action) => {
  switch (action.type) {
    ...
    case 'CHOOSE_CATEGORY':
      ...
      const newCurrentCategory = categories.find(category => category.id === action.categoryId);
      if (!newCurrentCategory) throw Error(`no such categoryId: ${action.categoryId})`);
      // decide which sub category to show
      const newSubCategoy = subCategories.find(subcategory => subcategory.id === action.categoryId);
      if (!newSubCategoy)
        throw Error(`no such categoryId in subcategory Array: ${action.categoryId})`);
    ...
    case 'CHOOSE_SUB_CATEGORY':
      ...
      if (!newCurrentSubCategory)
        throw Error(`no such subcategoryId in sub category Array: ${action.subCategoryId}`);
      ...
      if (!newSubSubCategory)
        throw Error(`no such subcategoryId is sub sub category Array: ${action.subCategoryId}`);
...
export const NavCategoryContext = createContext<any>(null);
const NavCategoryProvider: React.FC<React.ReactNode> = ({ children }) => {
  const [state, dispatch] = useReducer(categoryReducer, initState);
  const value = { state, dispatch };
  return <NavCategoryContext.Provider value={value}>{children}</NavCategoryContext.Provider>;
};
