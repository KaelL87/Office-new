...
const apiErrorMessage = (error: AxiosError) => {
  ...
  try {
    if (response && typeof response === 'object') {
      const message =
        (response.data &&
          (response.data.message || response.data.errorMessage || response.data.errors)) ||
        response.data ||
        response;
...
export const errorHandler = (error: AxiosError): void => callErrorToast(apiErrorMessage(error));
